...
4. Procurar a sua distro de preferência. Algumas das distros disponíveis são:
* Ubuntu
* OpenSuse
* Kali Linux
* Debian
...
E dando certo o botão da direita vai virar “iniciar” e agora você pode usar o ubuntu no seu windows
Se você procurar no menu do windows pelo nome da distro que você instalou, já deve estar disponível pra você.

6.  Abra a distro que você instalou.

A primeira vez que você acessa o seu subsistema linux, aparece uma notificação que ele está instalando, e pouco depois ele pede para você cadastrar sua conta no linux, pedindo o nome do seu usuário e senha. 
Esses dados não necessariamente são iguais aos do windows. Você pode colocar o que quiser. 

![imagem do terminal do ubuntu no windows depois de instalar o ubuntu e pedindo usuário e senha para primeira configuração](/img/elixir-windows/04.png)
...
7. Faça a atualização do seu linux com o comando sudo apt update.
...
![A imagem foi tirada diretamente do site do aws e mostra o menu de como instalar usando 'bash & git', no qual ele pede para adicionar duas linhas no arquivo .bashrc](/img/elixir-windows/07.png)

6. Vamos abrir o arquivo solicitado no terminal, usando o vim. Para isso digitamos `vim ~/.bashrc` no terminal.

7. Usando as setas do teclado vamos ao final do documento APERTAMOS iI para entrar no modo de inserção, e colamos os dois comandos solicitados.

8. Depois disso precisamos salvar o arquivo. Apontamos esc > ditamos :wq > enter.

9. Para as alterações funcionarem, é preciso reiniciar o shell. Podemos fazer isso fechando e abrindo o terminal.

Para saber se funcionou, digitamos “asdf”. Uma tela similar a essa deve aparecer:

![A imagem é uma captura de tela do terminal mostrando a documentação do asdf que aparece logo após digitarmos o comando 'asdf'](/img/elixir-windows/07.png)
...
4. Agora que instalamos as dependencias, instalamos o plugin do erlang usando `asdf plugin add erlang https://github.com/asdf-vm/asdf-erlang.git` (o commando também pode ser encontrado no repositório do asdf-erlang)
...
1. Os passos que seguimos no elixir são bem similares aos da instalação do erlang. A diferença é que é muito mais rápido, e elixir não tem pré-requisitos de instalação no sistema. 

2. Vamos instalar o plugin de elixir do asdf com `asdf plugin add elixir`
...
2. Vamos instalar o plugin de elixir do asdf com `asdf plugin add nodejs`

3. Vamos instalar a última versão digitando `asdf install nodejs latest`

4. Vamos definir a versão que queremos usar de elixir com `asdf global nodejs latest`

Para checar se deu certo, podemos usar o `asdf list nodejs`
...
O numero que vem depois do “otp” é a versão do erlang que aquela versão do elixir suporta.
